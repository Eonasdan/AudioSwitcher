...
{
    private static readonly Dictionary<string, DeviceIcon> IconMap = new()
    {
    ...
    private static DeviceIcon IconStringToDeviceIcon(string iconStr)
    {
        try
        {
            var imageKey = iconStr.Substring(iconStr.IndexOf(",", StringComparison.InvariantCultureIgnoreCase) + 1)
                .Replace("-", "");
    ...
    private void LoadAudioMeterInformation()
    {
        ...
        try
        {
            ...
            ex = Marshal.GetExceptionForHR(Device.Activate(ref clsGuid, ClassContext.Inproc, IntPtr.Zero,
                out var result));
            ...
            _audioMeterInformation = new ThreadLocal<IAudioMeterInformation>(() =>
                Marshal.GetUniqueObjectForIUnknown(_audioMeterInformationPtr) as IAudioMeterInformation);
        }
        catch (Exception e)
        {
            ex = e;
