...
public sealed partial class CoreAudioDevice : Device
{
    ...
    private static readonly Dictionary<PropertyKey, HashSet<string>> PropertykeyToPropertyMap = new()
    {
        { PropertyKeys.PKEY_DEVICE_INTERFACE_FRIENDLY_NAME, [nameof(InterfaceName)] },
        { PropertyKeys.PKEY_DEVICE_DESCRIPTION, [nameof(Name), nameof(FullName)] },
        { PropertyKeys.PKEY_DEVICE_FRIENDLY_NAME, [nameof(FullName)] },
        { PropertyKeys.PKEY_DEVICE_ICON, [nameof(Icon), nameof(IconPath)] }
    };

    private readonly CoreAudioController _controller;
    private readonly ManualResetEvent _defaultCommResetEvent = new(false);
    private readonly ManualResetEvent _defaultResetEvent = new(false);
    private readonly AutoResetEvent _muteChangedResetEvent = new(false);
    private readonly SemaphoreSlim _setDefaultCommSemaphore = new(1);

    private readonly SemaphoreSlim _setDefaultSemaphore = new(1);
        ...
        : base(controller)
    {
        ...
        _device = new ThreadLocal<IMultimediaDevice>(() =>
            Marshal.GetUniqueObjectForIUnknown(devicePtr) as IMultimediaDevice);
        ...
        controller.SystemEvents.DefaultDeviceChanged
            .When(x =>
            {
                ...
                return (x.DeviceRole == ERole.Communications && _isDefaultCommDevice) ||
                       (x.DeviceRole != ERole.Communications && _isDefaultDevice);
            })
