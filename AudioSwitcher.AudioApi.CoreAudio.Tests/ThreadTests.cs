...
public class ThreadTests
{
    ...
    public void CoreAudio_Attempted_Thread_Deadlock()
    {
        ...
        {
            controller.AudioDeviceChanged.Subscribe(x => { Console.WriteLine(x.ToString()); });

            for (var i = 0; i < 50; i++) controller.DefaultPlaybackDevice.SetAsDefault(CancellationToken.None);
    ...
    public async Task CoreAudio_Attempted_Thread_Deadlock_Async()
    {
        ...
        using (var controller = CreateTestController())
        {
            ...
            for (var i = 0; i < 50; i++) tasks.Add(controller.DefaultPlaybackDevice.SetAsDefaultAsync());
